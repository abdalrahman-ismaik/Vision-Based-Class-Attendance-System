...
FAMOUS_PEOPLE_FOLDER = Path(__file__).parent / "famous-people"
...
def generate_student_id(name):
    """Generate a student ID from name"""
    # Remove spaces and use first 8 chars + number
    clean_name = name.replace(" ", "").replace("-", "")[:6].upper()
    return f"FP{clean_name}{random.randint(100, 999)}"
        ...
        for idx, image_filename in enumerate(test_images, 1):
            ...
            try:
                with open(image_path, 'rb') as img_file:
                    files = {'images': img_file}  # API expects 'images' field name
...
def cleanup_test_data():
    ...
    print("   1. Delete the database.json file")
    print("   2. Clear the processed_faces folder")
    print("   3. Clear the classifiers folder")
    print("\n⚠️  Run these commands manually if needed:")
    print("   rm backend/database.json")
    print("   rm -r backend/processed_faces/*")
    print("   rm -r backend/classifiers/*")
